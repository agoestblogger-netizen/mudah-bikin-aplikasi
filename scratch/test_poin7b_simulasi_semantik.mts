...
for (const row of simEsKrim.contohData.baris) {
...
const mEs = validateContohDataVsSchema(simEsKrim.contohData, esKrim.dataSchema!.tabel[0]);
...
console.log(`  Tabel terpilih: ${simRental.contohData.tabel}`);
for (const row of simRental.contohData.baris) {
...
checkNoPlaceholder(simRental.contohData.baris, 'Rental-Armada');
assert.strictEqual(validateContohDataVsSchema(simRental.contohData, rentalArmada.dataSchema!.tabel[0]).length, 0);
...
for (const row of simRentalT.contohData.baris) {
...
checkNoPlaceholder(simRentalT.contohData.baris, 'Rental-Transaksi');
assert.strictEqual(validateContohDataVsSchema(simRentalT.contohData, rentalTransaksi.dataSchema!.tabel[0]).length, 0);
...
for (const row of simKoperasi.contohData.baris) {
...
checkNoPlaceholder(simKoperasi.contohData.baris, 'Koperasi');
assert.strictEqual(validateContohDataVsSchema(simKoperasi.contohData, koperasi.dataSchema!.tabel[0]).length, 0);
...
for (const row of simRosok.contohData.baris) {
...
checkNoPlaceholder(simRosok.contohData.baris, 'Rosok');
assert.strictEqual(validateContohDataVsSchema(simRosok.contohData, rosok.dataSchema!.tabel[0]).length, 0);
...
for (const row of simSembako.contohData.baris) {
...
checkNoPlaceholder(simSembako.contohData.baris, 'Sembako');
assert.strictEqual(validateContohDataVsSchema(simSembako.contohData, sembako.dataSchema!.tabel[0]).length, 0);
